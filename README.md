...
| 目标定位 | **行程级连通团块跟踪**（`blob_track`）：把每行掩码的连通行程抽出来，与 4 个团块记录做「水平重叠 + 行相邻」匹配（命中多个就合并），帧末取面积最大者。**抗反光**：反射在灯上打出的洞不会把框切成两半；且能同时看到多个目标 |
| 圆形状校验 | 外框宽高都 ≥ `MIN_SIZE`、面积 ≥ `MIN_AREA`、**填充率 `area/(w*h)` 接近 π/4**（`fill_q8` 圆约 201）|
| 瞄准点 | 打击点在绿灯**上方一点** → 把圆心往上偏移 `宽度 × AIM_H_Q8/256` 像素（RM 靶标 = 372，即 1.449 倍灯直径）。**不需要额外测距**，距离已被「表观宽度」隐含（推导见 §7） |
...
所以只要标定这个比例（`AIM_H_Q8`，**16 位** Q0.8 定点），就能直接算瞄准点，
不需要单独测距 —— 距离信息已经隐含在宽度里了。

> **RM 飞镖靶标的实测值**：灯直径 55mm、打击点在灯心上方 80mm
> → `AIM_H_Q8 = round(256 × 80/55) = 372`（= 1.449 倍灯直径），**这就是现在的默认值**。
> 它大于 255，所以这个参数必须是 **16 位** —— 早期 8 位版本会把 372 静默截断成 116（= 0.45 倍），
> 而 `blob_track.v` 里那个没写位宽的 parameter 更隐蔽（xor 都不报）。
...
| `AIM_H_Q8` | **372**（16 位） | 瞄准点在灯心上方 = 宽度 × (AIM_H_Q8/256)。RM 靶标：80mm ÷ 55mm → 372 = 1.449 倍灯直径。**现场标定**：量「打击点到灯心」÷「灯直径」× 256 |
...
偏移量是 1.449 倍灯宽，比红环半径（0.5 倍灯宽）大得多，所以 **相机必须对准打击点、而不是对准灯**：
灯自然落在画面下方约 1.45 个灯宽处，瞄准点才落在画面中心。约束是

$$1.449\,W \;\le\; c_y \;\le\; 479 - W/2 \quad\Longrightarrow\quad W \le 245\text{ px}$$

推荐灯的表观宽度做 **100~130 px**（灯心 y ≈ 390~420），此时瞄准点正好在画面中心。

- 若把灯放在画面正中（`c_y ≈ 240`）、灯宽 200 px，瞄准点会被算到画面**上方之外**，
  此时 `aim_y` **饱和到 0**（不回绕）。这就是 `blob_track.v` 用饱和减法而不是直接 `cy - up` 的原因：
  无符号下溢会回绕到 1023 附近，十字会跳到画面底部。
- 飞行中弹体会滚转，图像的「上方」不再等于物理上方 → 偏移方向要由 PS 侧用 IMU 的 roll 角修正
  （PL 只给图像坐标系的结果，这也是把它放在 PL 的合理边界）。
- 越飞越近时 $W$ 变大：比例形式自动跟随（这正是不能用固定像素偏移的原因）。
  但灯贴到画面左右边缘时 $W$ 会被裁短、距离代理量失效 —— 需要时可按 `bond_l==0 || bond_r==满幅` 判无效。
...
| **十字贴到画面顶端 / 跑出画面** | 正常现象：偏移 = 1.449 倍灯宽，比灯心到画面顶端的距离还大 → 说明**取景没对准打击点**。把相机抬起（或把灯放低），让灯心落在 $c_y \approx 1.45W$ 处 |
...
| **综合或实现报 `[DRC MDRV-1] Multiple Driver Nets`，但仿真明明是过的** | 同一个 `reg` 被**两个 `always` 块**驱动。**xsim 容忍这种写法（自己挑一个驱动），Vivado 直接拒绝**，所以「仿真全过」不等于能实现。已修的两处：`chroma_hist.v` 的 `hgr`/`hgb`/`total`（把清零与累加合成**单写端口**）、`osd_text.v` 的 `conv_done`（只留状态机驱动，并改成单拍脉冲）。自查：对每个时序块写的 reg，全局搜它出现几次 `<=` 赋值，跨块就是多驱动。修复脚本 `python tools/patch_fix_multidriver.py`，细节见 `doc/vision_pipeline.md` §18 |
...
| `add_sources_vivado.tcl` | **注册新源文件的正解（在 Vivado 的 Tcl Console 里 `source` 它）**。为什么必需：工程在 Vivado 里打开时，任何一次保存都会用**内存状态覆盖磁盘上的 `.xpr`**，所以在外部手改 `.xpr` 的注册会被静默冲掉（实测：11 个新模块的注册全被抹掉，综合报 `[Synth 8-439] module 'uart_rx' not found`） |
| `add_source.py` | 新增 `rtl/*.v` 的转码 + 注册（UTF-8 → GBK，回读比对；并写 `.xpr`）。**转码部分可靠；`.xpr` 那半只能在 Vivado 关着时用** —— 否则请用上面的 TCL |
...
| `AIM_H_Q8` | **372**（16 位） | 瞄準點在燈心上方 = 寬度 × (AIM_H_Q8/256)。RM 靶標：80mm ÷ 55mm → 372 = 1.449 倍燈直徑。**現場標定**：量「打擊點到燈心」÷「燈直徑」× 256 |
...
| **十字貼到畫面頂端 / 跑出畫面** | 正常：偏移 = 1.449 倍燈寬，比燈心到畫面頂端的距離還大 → 取景沒對準打擊點。把相機抬起（或把燈放低），讓燈心落在 $c_y \approx 1.45W$ 處 |
...
| **合成或實現報 `[DRC MDRV-1] Multiple Driver Nets`，但模擬明明是過的** | 同一個 `reg` 被**兩個 `always` 塊**驅動。**xsim 容忍這種寫法（自己挑一個驅動），Vivado 直接拒絕**，所以「模擬全過」不等於能實現。已修的兩處：`chroma_hist.v` 的 `hgr`/`hgb`/`total`（把清零與累加合成**單寫埠**）、`osd_text.v` 的 `conv_done`（只留狀態機驅動，並改成單拍脈衝）。自查：對每個時序塊寫的 reg，全域搜它出現幾次 `<=` 賦值，跨塊就是多驅動。修復腳本 `python tools/patch_fix_multidriver.py`，細節見 `doc/vision_pipeline.md` §18 |
