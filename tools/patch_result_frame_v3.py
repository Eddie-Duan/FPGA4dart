# -*- coding: utf-8 -*-
"""
patch_result_frame_v3.py -- 上报帧升级到 v3（34 字节 + CRC16 + 帧序号）

为什么要改（P11 配套）：
  1) 云台真正需要的是「该指向哪里」——把预测灯心、几何偏移、弹道下坠补偿
     全算好后给一个 fx/fy，接收端不用重算（中间量也一起发，便于调试）。
  2) 距离（dist_cm）与下坠（drop_px）是新增的弹道字段。
  3) 原来的 1 字节异或校验只能查单字节错；改成 **CRC-16/CCITT-FALSE**，
     并加 **帧序号 seq**（接收端能发现丢帧，云台指令链路必须知道这个）。

新帧（v3 = 34 字节，115200 8N1）：
   0  A5 / 1  5A
   2  flags  b7=1(扩展帧) b6=far b5=moving b4=pred_ok b3=dist_ok
             b2=二值显示 b1=adapt_ok b0=valid
   3-4 cx  5-6 cy  7-8 ax  9-10 ay   11 w  12 area>>7  13 fill_q8  14 blob_cnt
   15-16 vx_q4   17-18 vy_q4   19-20 px   21-22 py
   23-24 dist_cm  25-26 drop_px  27-28 fx  29-30 fy   31 seq   32-33 crc16
   （ax/ay 仍是「当前帧的几何瞄准点」，语义没变；云台请用 fx/fy）

CRC 用逐位串行算（240 拍），不做 256 级组合异或 —— 一帧 3ms 内算完，不影响时序。

文件是 GBK：本脚本整文件重写（内容全部由 Python 以 GBK 编码写出并回读比对）。
"""
...
def main():
    data = VERILOG.replace('\n', '\r\n').encode('gbk')   # 编不了会在这里炸，是好事
    path = os.path.join(ROOT, REL)
    with open(path, 'wb') as fh:
        fh.write(data)
    with open(path, 'rb') as fh:
        back = fh.read().decode('gbk')
    if back != VERILOG.replace('\n', '\r\n'):
        raise SystemExit('**** 回读比对失败 ****')
    print('[ok]   %s  重写为 GBK，%d bytes' % (REL, len(data)))
